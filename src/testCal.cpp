 ...
 /* for ( int i=0; i<imgs.size(); i++ )
  {
      ...
      information_CtoW(0,0) = information_CtoW(1,1) = information_CtoW(2,2) = 10; // 100
      information_CtoW(3,3) = information_CtoW(4,4) = information_CtoW(5,5) = 1000; // ; 1000000000000000; // 不考虑translation了
...
/*
  for (int idx =1;idx< imgs.size();idx++){
    ...
    Eigen::Matrix<double, 6, 6> information_fix = 1000*Eigen::Matrix< double, 6,6 >::Identity();
...
*/
  ...
  // check_error(optimizer, M, imgs.size()); 
  check_error(vedge); 
